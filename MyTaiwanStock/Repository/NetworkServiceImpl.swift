...
class NetworkServiceImpl: NetworkService {
    ...
    internal func fetchCandleData(
        stockNo: String,
        dateStr: String,
        completion: @escaping (Result<StockInfo, Error>) -> Void
    ) {
        var urlComponents = URLComponents(
        ...
        urlComponents.queryItems = [
            "date": dateStr, "response": "json", "stockNo": stockNo,
        ]
    ...
    func fetchStockInfo(stockNo: String, dateStr: String = "20210930") async
    ...
    func fetchTwoMonthCandleData(stockNo: String) async -> [[String]] {
        ...
        for dateStr in dateStrs {
            ...
            let result = await fetchStockInfo(
                stockNo: stockNo,
                dateStr: dateStr
            )
    ...
    func saveList(with listName: String) -> List {
        guard let list = localDBService.saveNewListToDB(listName: listName)
        else { fatalError("unable to create list in db") }
    ...
    func saveStockNumber(
        with stockNumber: String,
        currentFollowingList: ListStruct
    ) {
        localDBService.saveNewStockNumberToDB(
            stockNumber: stockNumber,
            currentFollowingList: currentFollowingList
        )
        guard let listName = currentFollowingList.name else { return }
        print("saveStockNumber listname \(listName)")
        onLineDBService.uploadNewStockNoToOnlineDB(
            stockNumber: stockNumber,
            listName: listName
        )
    }
    ...
    func saveCashDividend(stockNo: String, amount: Int, date: Date) {
        localDBService.saveCashDividend(
            stockNo: stockNo,
            amount: amount,
            date: date
        )
    ...
    func updateStockNoInDBwithPrice(
        stockNos: [String],
        cellViewModels: [StockCellViewModel]
    ) {
        localDBService.updateStockNoInDBwithPrice(
            stockNos: stockNos,
    ...
    func sendDeviceIdToServer(deviceId: String, token: String) {
        let url = URL(
            string: "http://albertkingdom.ddns.net:3000/register-device"
        )!
        ...
        let task = URLSession.shared.dataTask(with: request) {
            ...
            if let error = error {
                print(
                    "Error sending device ID to server: \(error.localizedDescription)"
                )
                return
            }
    ...
    private func startDateOfMonth(diff: Int) -> Date {
        ...
        //print(datecomponent)
        let lastmonth = Calendar(identifier: .gregorian).date(
