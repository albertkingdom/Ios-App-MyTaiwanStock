...
- [ ] 3.1 新增 build step，以 `xcodebuild build -scheme MyTaiwanStock -sdk iphonesimulator -destination 'platform=iOS Simulator,name=iPhone 16' CODE_SIGNING_ALLOWED=NO` 建置 App target，實現規格「Build without code signing」與設計決策「建置目的地固定為 iOS Simulator（`platform=iOS Simulator,name=iPhone 16`），並停用程式碼簽章」。驗證方式：觸發 workflow run，確認該 step 結束碼為 0 且 log 無簽章相關錯誤（不出現 "requires a provisioning profile" 等訊息）。
